...
function breadcrumb_trail( $args = array() ) {
    ...
    $packers_movers_company_breadcrumb = apply_filters( 'breadcrumb_trail_object', null, $args );

    if ( ! is_object( $packers_movers_company_breadcrumb ) )
        $packers_movers_company_breadcrumb = new Breadcrumb_Trail( $args );

    return $packers_movers_company_breadcrumb->trail();
}
...
class Breadcrumb_Trail {
    ...
    public function trail() {
        ...
        if ( 0 < $item_count ) {
            ...
            if ( true === $this->args['show_browse'] ) {
                ...
                $packers_movers_company_breadcrumb .= sprintf(
            ...
            $packers_movers_company_breadcrumb .= sprintf( '<meta name="numberOfItems" content="%d" />', absint( $item_count ) );
            $packers_movers_company_breadcrumb .= '<meta name="itemListOrder" content="Ascending" />';
            ...
            foreach ( $this->items as $item ) {
                ...
                // Build the list item.
                $packers_movers_company_breadcrumb .= sprintf( '<%1$s %2$s>%3$s%4$s</%1$s>', tag_escape( $this->args['item_tag'] ),$attributes, $item, $meta );
            }

            // Close the unordered list.
            $packers_movers_company_breadcrumb .= sprintf( '</%s>', tag_escape( $this->args['list_tag'] ) );
        ...
        $packers_movers_company_breadcrumb = apply_filters( 'breadcrumb_trail', $packers_movers_company_breadcrumb, $this->args );

        if ( false === $this->args['echo'] )
            return $packers_movers_company_breadcrumb;

        echo $packers_movers_company_breadcrumb;
    }
    ...
    protected function set_labels() {
        ...
        $defaults = array(
            'browse'              => esc_html__( 'Browse:',                               'packers-movers-company' ),
            'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'packers-movers-company' ),
            'home'                => esc_html__( 'Home',                                  'packers-movers-company' ),
            'error_404'           => esc_html__( '404 Not Found',                         'packers-movers-company' ),
            'archives'            => esc_html__( 'Archives',                              'packers-movers-company' ),
            // Translators: %s is the search query.
            'search'              => esc_html__( 'Search results for: %s',                'packers-movers-company' ),
            // Translators: %s is the page number.
            'paged'               => esc_html__( 'Page %s',                               'packers-movers-company' ),
            // Translators: %s is the page number.
            'paged_comments'      => esc_html__( 'Comment Page %s',                       'packers-movers-company' ),
            // Translators: Minute archive title. %s is the minute time format.
            'archive_minute'      => esc_html__( 'Minute %s',                             'packers-movers-company' ),
            // Translators: Weekly archive title. %s is the week date format.
            'archive_week'        => esc_html__( 'Week %s',                               'packers-movers-company' ),
    ...
    protected function add_minute_hour_archive_items() {
        ...
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'packers-movers-company' ) ) );
    }
    ...
    protected function add_minute_archive_items() {
        ...
        // Add the minute item.
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'packers-movers-company' ) ) );
    }
    ...
    protected function add_hour_archive_items() {
        ...
        // Add the hour item.
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'packers-movers-company' ) ) );
    }
    ...
    protected function add_day_archive_items() {
        ...
        $year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'packers-movers-company' ) ) );
        $month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'packers-movers-company' ) ) );
        $day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'packers-movers-company' ) ) );
    ...
    protected function add_week_archive_items() {
        ...
        $year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'packers-movers-company' ) ) );
        $week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'packers-movers-company' ) ) );
    ...
    protected function map_rewrite_tags( $post_id, $path ) {
        ...
        if ( is_array( $matches ) ) {
            ...
            foreach ( $matches as $match ) {
                ...
                if ( '%year%' == $tag )
                    $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'packers-movers-company' ) ) ) );

                // If using the %monthnum% tag, add a link to the monthly archive.
                elseif ( '%monthnum%' == $tag )
                    $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'packers-movers-company' ) ) ) );

                // If using the %day% tag, add a link to the daily archive.
                elseif ( '%day%' == $tag )
                    $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'packers-movers-company' ) ) ) );
